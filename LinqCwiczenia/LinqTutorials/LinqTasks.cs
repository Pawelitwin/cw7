...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            var result = Emps.Where(emp => emp.Job == "Frontend programmer" && emp.Salary > 1000)
        ...
        public static IEnumerable<object> Task11()
        {
            var result = Emps.GroupBy(emp => emp.Deptno)
                             ...
                             .Select(group => new
                             {
                                 name = Depts.FirstOrDefault(d => d.Deptno == group.Key)?.Dname,
        ...
        {
            var result = Depts.Where(dept => Emps.Count(emp => emp.Deptno == dept.Deptno) == 5 ||
        ...
        {
            var result = Emps.Where(emp => emp.Job.Contains('a') || emp.Job.Contains('A'))
                             .GroupBy(emp => emp.Job)
        ...
        {
            var result = Emps.Select(emp => new { emp, dept = Depts.FirstOrDefault(d => d.Deptno == emp.Deptno) })
                             .Select(empDept => new
                             {
                                 empDept.emp.Empno,
                                 empDept.emp.Ename,
                                 ...
                                 empDept.emp.Mgr,
                                 empDept.emp.Salary,
                                 empDept.emp.HireDate,
                                 empDept.emp.Deptno,
                                 DeptDname = empDept.dept?.Dname,
                                 DeptLoc = empDept.dept?.Loc
                             });
            return result;
        }
    }
    ...
    public static class CustomExtensionMethods
    {
        ...
        {
            var result = emps.Where(emp => emps.Any(sub => sub.Mgr == emp))
